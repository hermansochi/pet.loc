...
import { healthcheck, headers } from "./patch"; // константы путей
...
function App() {
  ...
  const url = process.env.REACT_APP_API_URL; // основной путь
  const v = process.env.REACT_APP_API_VER; // версия api

  useEffect(() => {
    // запрос для проверки соединения
    fetch(url + "/" + v + "/" + healthcheck, {
      method: "GET",
      headers,
    })
